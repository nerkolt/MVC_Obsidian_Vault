...
Let's say you have 3 cars in split-screen:
```csharp
Scene Setup:
...
---
**Runtime Plugin Discovery** At startup, `PluginInputBootstrapper` uses **reflection** to find all classes that extend `PluginInputProvider`. You don't need to register plugins manually - they're auto-discovered!

---
## Common Questions

>**Q: Why use a hash instead of the plugin class directly in settings?** 
>A: Serialization. You can't serialize a Type directly in Unity, but you can serialize an int. The hash is just `GetType().GetHashCode()`.

>**Q: What about AI or networked vehicles?** 
>A: They bypass this entire system! They write directly to `Powertrain.Inputs`. Only player-controlled vehicles use the plugin system.

>**Q: Why is `PlayerIdentifier` in Core, not `InputProviders`?** 
>A: Because it's a general vehicle component. Other systems might need to know "which player owns this vehicle" for things like camera assignment, UI, etc.

>**Q: How does gear shifting work?** 
>A: Plugins declare which modes they support via `GetAllowedGearShiftControlTypes()`. If a vehicle is in Manual mode but the plugin doesn't support it, you get a clear error telling you which method to implement.